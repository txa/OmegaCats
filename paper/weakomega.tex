...
\begin{definition}
A \emph{weak $\omega$ category} is given by the following well founded
definition:
\begin{enumerate}
\item A context $\Gamma : \mathsf{Con}$
\item A globular set $G : \Glob$ 

\item An interpretation globular set morphism $\intpr{\text{-}} :
  \mathsf{glob}~\Gamma \to G$ :
\[
\AxiomC{$\{C : \Cat ~ \Gamma\}$}
\AxiomC{$x : \Obj~C$}
\BinaryInfC{$\intpr{x} : \obj~(\intpr{C}^\Cat~\intpr{\Gamma}^\Con)$}
\DisplayProof
\]
such that
\[
\AxiomC{$\{C : \Cat~\Gamma\}$}
\AxiomC{$v : \mathsf{Var}~C$}
\BinaryInfC{$\intpr{\var~v} ~=~ \intpr{v}^\var~\intpr{\Gamma}^\Con$}
\DisplayProof
\qquad
...
\DisplayProof
\]
and where we assume the following definitions
% as follows:
\[
\AxiomC{$\{ C : \Cat~\Gamma \}$}
\UnaryInfC{$\intpr{\mathsf{vz}}^\var = \pi_2
\qquad \intpr{\mathsf{vs}~v}^\var = \intpr{v}^\var \circ \pi_1$}
\DisplayProof
\]
% and $\intpr{\text{-}}^\Con$ is defined as follows:
\[
\AxiomC{$\phantom{\{\}}$}
\UnaryInfC{$\intpr{\varepsilon}^\Con = 1
\qquad
\intpr{\Gamma, C}^\Con = \Sigma(\overline{x} :
\intpr{\Gamma}^\Con)(\intpr{C}^\Cat~\overline{x})$}
\DisplayProof 
\]
\[
\AxiomC{$\{\Gamma : \Con\}$}
\AxiomC{$C : \Cat~\Gamma$}
\AxiomC{$\overline{x} : \intpr{\Gamma}^\Con$}
\TrinaryInfC{$\intpr{\bullet}^\Cat ~ \overline{x} = G
\qquad
\intpr{\homcat{C}{a}{b}}^\Cat ~ \overline{x} = (\mathsf{hom} ~
(\intpr{C}^\Cat~\overline{x}))~(\intpr{a}~\overline{x})~(\intpr{b}~\overline{x}) 
$}
\DisplayProof
\]


\item a globular set morphism
\[ s : G \longrightarrow \mathsf{glob}~\Gamma \]
which is a section of $\intpr{\text{-}}$. 
\end{enumerate}
\end{definition}
%
\mnote{We might need that each homset is a set, and we should build
  this into the definition of globular sets, and prove that our
  interpretation is such. Otherwise everything seems so loose. I
  believe this will crop up as soon as we try to compare our
  definition to a standard one. }
%
Explicitly, a globular set morphism
$\glob{f}{g} : \glob{O}{H} \to \glob{O'}{H'}$ is a \emph{section} of a
morphism $\glob{h}{k} : \glob{O'}{H'} \to \glob{O}{H}$ if $f$ is a
section of $h$, i.e.:
\[
(x : O) \rightarrow (h (f x) \equiv x)
\]
