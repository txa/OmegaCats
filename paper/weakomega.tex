...
%
In the following text we usually drop the index $\meets$ on
$\zeromeets$ and $\sucmeets{}$ and write elements of $C\,\meets\,D$ just
as natural numbers. 
Note that it is readily provable by induction, and  using globularity,
that 
\mnote{This is essential and we also need the opposite direction }
\begin{align*}
...
\DisplayProof
\]
Here, dots indicate repetition of all assumptions from the
previous rule. 
\[
%\AxiomC{$\{\Gamma\}$}
\AxiomC{$C ~ D : \Cat ~\Gamma$}
...
\]
\[
\AxiomC{$\phantom{G}\cdots\phantom{G}$}
\UnaryInfC{$\id\,0\,b = b \qquad \id\,(n+1)\,b = \id\,(\id\,n\,b)$}
\DisplayProof
\]

\mnote{This looks like deserving a closed (noniterative)
  form.  I don't suppose it's possible just now, but maybe if one
  tried to do it some interesting connections would crop up.}
%
...
paper: \emph{an operator on objects mutually recursively extends to an
...
\begin{definition}
A \emph{weak $\omega$ category} is given by:
\begin{enumerate}
\item A context $\Gamma : \mathsf{Con}$
\item A \emph{globular set} $S$ such that each
  $S_n$ is a set in Voevodsky's sense \cite{}. \mnote{I want a proper
    Set and the only way to say it in Type Theory is by contractibility.}This means explicitly
  that we have the following data
\begin{align*}
  S &~:~ \Nat \rightarrow \Set \\
  \domrm &~:~ S \rightarrow S \\
  \codrm &~:~ s \rightarrow S 
\end{align*}
and an axiom
\begin{equation}\label{eq:contr}
 (n : \Nat)\rightarrow (x~y: S~n)\rightarrow\Sigma(p : x \equiv y)((q : x \equiv
y)\rightarrow (p\equiv q)) 
\end{equation}
which is saying that propositional equality in each $(S~n)$ is contractible.
\item An \emph{interpretation function} 
\[\llbracket \text{-} \rrbracket : \big(\Sigma (C :
\Cat~\Gamma)(\Obj~C)\big) \rightarrow S\]
which is a map of globular sets in the sense that for all $(x : \Obj ~\homcat{C}{a}{b})$:
\begin{align*}
  \domrm \intpr{x} &\;\equiv\; a &\codrm \intpr{x} &\;\equiv\; b
\end{align*}
\item a \emph{function} 
\[ s : S \rightarrow \big(\Sigma (C :
\Cat~\Gamma)(\Obj~C)\big) \]
which is a section\mnote{It is going to be something along these lines
but what is it exactly?} of $\llbracket\text{-}\rrbracket$. Explicitly,
%
\begin{equation}
(x : X) \rightarrow ( \intpr{s~x} \equiv x)\enspace.
\end{equation}
%
\end{enumerate}
\end{definition}
...
total. Moreover, and importantly, it implies that $S$ carries the
...
$\intpr{\text{-}}$ is coherent with it. For example, to define composition
operation on $S$:
\[
\mathrm{comp} : (x~y : S) \rightarrow \Sigma(n : \Nat)(\domrm^n~ x
\equiv \codrm^n~x) \rightarrow S 
\]
we put
\[
\mathrm{comp} ~x~y \quad=\quad \intpr{ \mathsf{comp}~(s \, x)~(s \,
  y)~(s \ldots)}
\]
\begin{ondrej}
  We want to show that whenever $\domrm^n~ x
\equiv \codrm^n~x$, then ${s~x}\meets{s~y}$.
\end{ondrej}
